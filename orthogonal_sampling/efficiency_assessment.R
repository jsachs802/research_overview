...
## Test variances of bootstraps 
variances <- vector(mode="numeric", length = iters)
for(i in 1:length(iters)){
  
  variances[[i]] <- var(estimate_objects[[i]][[3]])
  
}

## Bootstrap variance of the variances




save(variances, file = paste0(root, "orthogonal_sampling_project/", folder, "/Data/variances.Rda"))
save(seeds, file = paste0(root, "orthogonal_sampling_project/", folder, "/Data/var_seeds.Rda"))