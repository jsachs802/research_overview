simulator_fn <- function(seed_list = seed_list, 
                         vill = vill,
                         t = t,
  ...
  susceptible[[1]] <- seed_list ## seed strategy
  sus_nodes[[1]] <- village_nodes$i[village_nodes$i %in% susceptible[[1]]] ## names of nodes that are susceptible
  sus_length[[1]] <- length(sus_nodes[[1]]) ## number of nodes that are susceptible
  sus_cov[[1]] <- village_nodes[village_nodes$i %in% sus_nodes[[1]],] ## subset of covariates of susceptible nodes 
  ...
  ### REPLACE NULL VALUES IN ADOPTED AND HEARD
  
  ### do baseline outputs -- other manipulations can be done for fairness, moments, etc. in separate functions
  
  
  return(list(sim_df = simulation_df_fn(a = adopted, ear = heard, nodes = nodes_df), cum_rates = cum_rates_fn(a = adopted, ear = heard, vill_df = vill_df, t = t)))

  # return(list(adopted, heard))
  # 
...
simulation_df_fn <- function(a = adopted, ear = heard, nodes = nodes_df){
  ...
  ### Should add a seed variable 
  
} ## could add a simulation variable** 


cum_rates_fn <- function(a = adopted, ear = heard, vill_df, t){
  
  period = 1:t
  
  # cum_adopt = sapply(1:length(a), function(x){
  #   return(length(a[x]))
  #   })
  
  cum_heard = sapply(1:t, function(x){
    return(length(ear[[x]][!is.na(ear[[x]])]))
  })
  
  cum_heard_male = sapply(1:t, function(x){
    return(length(ear[[x]][!is.na(ear[[x]]) & ear[[x]] %in% vill_df$i[vill_df$female == 0]]))
  })
  
  cum_heard_female = sapply(1:t, function(x){
    return(length(ear[[x]][!is.na(ear[[x]]) & ear[[x]] %in% vill_df$i[vill_df$female == 1]]))
  })
  
  tib = tibble(period = period, cum_heard = cum_heard, male = cum_heard_male, female = cum_heard_female)
